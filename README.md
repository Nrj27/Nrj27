<h1 align="center">Hi, I'm Neeraj Rikhari 👋</h1>

<h3 align="center">Backend Developer • Java • Python • APIs • Databases • Secure Systems</h3>

<p align="center">
  <img src="https://github.com/Nrj27/Nrj27/blob/main/matrix.svg" alt="Matrix animation" width="100%" />
</p>

<p align="center">
  <a href="https://linkedin.com/in/neerajrikhari">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://www.hackerrank.com/profile/neerajrikhari201">
    <img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="HackerRank" />
  </a>
  <a href="https://leetcode.com/u/Neeraj27/">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
  </a>
</p>

---

## About Me

I am a backend-focused developer passionate about building secure, scalable, and maintainable software systems. My primary interests are Java, Python, REST APIs, databases, backend architecture, and cybersecurity fundamentals.

- Building practical backend services and APIs
- Learning Spring Boot, system design, testing, and secure development
- Improving database design and application performance
- Open to collaboration, open-source work, and entry-level backend opportunities

---

## Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=java,python,spring,postgres,mysql,git,github,docker,linux,idea,vscode" alt="Java, Python, Spring, PostgreSQL, MySQL, Git, GitHub, Docker, Linux, IntelliJ IDEA, and VS Code" />
</p>

---

## Current Focus

- Backend development with Java and Python
- REST API design, validation, authentication, and error handling
- SQL, PostgreSQL, and MySQL database design
- Testing, Docker, Linux, and clean architecture
- Cybersecurity and secure software engineering

---

## GitHub Statistics

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=Nrj27&theme=tokyonight&hide_border=true" alt="Neeraj's GitHub contribution streak" width="49%" />
</p>


---


## Holopin Badges

[![Neeraj's Holopin board](https://holopin.me/neerajr_27)](https://holopin.io/@neerajr_27)

---

## Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/neerajrikhari">LinkedIn</a> •
  <a href="https://github.com/Nrj27">GitHub</a> •
  <a href="https://www.hackerrank.com/profile/neerajrikhari201">HackerRank</a> •
  <a href="https://leetcode.com/u/Neeraj27/">LeetCode</a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Nrj27&label=Profile%20views&color=6C63FF&style=flat" alt="Profile views" />
</p>

<p align="center"><em>Build thoughtfully. Learn continuously. Grow with the community.</em></p>
